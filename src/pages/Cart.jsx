      ...
      {cart.products.length > 0 ? (
        <div className="w-full max-w-screen-lg flex flex-col md:flex-row justify-around mt-8">
          <div className="w-full md:w-2/3 overflow-x-auto">
            ...
            <div className="mt-5">
              {cart.products.map((product, idx) => (
                <div key={idx} className="flex flex-col md:flex-row items-center bg-red-100 p-4 my-2 rounded-lg shadow-md">
                  <img className="h-20 w-20 object-cover rounded-md" src={product.image} alt={product.name} />
          ...
          <div className="w-full md:w-1/3 p-5 bg-red-200 mt-8 md:mt-0 rounded-xl shadow-lg lg:ml-10">
            ...
            <div className="w-full bg-red-50 h-px my-4"></div>
            <p className="text-yellow-800 mt-4">Shipping: Shipping to Chowkcity, Patna</p>
            <button className="w-full bg-red-500 mt-6 py-2 text-white rounded-full hover:bg-red-600">Proceed to Checkout</button>
          </div>
        </div>
      ) : (
